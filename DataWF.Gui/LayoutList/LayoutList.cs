...
namespace DataWF.Gui
{
    ...
    public partial class LayoutList : ScrollView, ILocalizable, ILayoutList
    {
        ...
        public virtual Rectangle GetCellTextBound(object item, LayoutColumn column, Rectangle cell)
        {
            var bound = cell.Inflate(-3, -2);
            bound.Y += 1;

            double indent = 0;
            if (column.Name == nameof(object.ToString))
            {
                var gitem = item as IGroup;
                ...
                if (AllowImage && item is IGlyph)
                {
                    var picture = (IGlyph)item;

                    if (gridCols > 1 && cell.Height > 36)
                    {
                        indent = 2;
                        bound.Y = cell.Bottom - cell.Height * 0.40F;
                        bound.Height = cell.Height * 0.40F;
                    }
