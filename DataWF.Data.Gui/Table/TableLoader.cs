        ...
        private void Loader()
        {
            ...
            if (queries.TryPop(out query))
            {
                ...
                try
                {
                    ...
                    using (var temp = new DBTransaction(view.Table.Schema.Connection) { View = view })
                    {
                        transaction = temp;
                        View.Table.LoadItems(temp, query, DBLoadParam.Synchronize);//DBLoadParam.GetCount | DBLoadParam.ReferenceRow                                            
                    }
                }
                catch (Exception e)
                {
