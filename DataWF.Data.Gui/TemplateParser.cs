					...
					else if (node.Type == PlaceholdeType.Image)
					{
						...
						if (img != null)
						{
							Frame frame = textDoc.GetFrame((byte[])pair.Value,
														   (img.Width / ((Xwt.Drawing.BitmapImage)img).PixelWidth) * 2.54D,
														   (img.Height / ((Xwt.Drawing.BitmapImage)img).PixelHeight) * 2.54D, 7);
							...
							foreach (var dr in pair.Value as List<Dictionary<string, object>>)
							{
								foreach (var dc in dr)
								{
									if (dc.Value.GetType() == typeof(byte[]))
									{
										...
										if (img != null)
										{
											Frame frame = textDoc.GetFrame((byte[])dc.Value, img.Width, img.Height, 7);
											img.Dispose();
											values.Add(frame);
										}
									}
						...
						foreach (var dr in query.Values)
						{
							...
							foreach (var item in dr)
							{
								...
								if (i < list.Count)
								{
									...
									{
										var img = (Xwt.Drawing.Image)item;
